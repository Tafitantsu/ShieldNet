...
if __name__ == '__main__':
    ...
    temp_log_dir = "temp_logs_colored"
    if not os.path.exists(temp_log_dir):
        os.makedirs(temp_log_dir)

    test_log_file = os.path.join(temp_log_dir, "test_app_colored.log")
    ...
    print(f"Check {test_log_file} and its rotated versions (e.g., test_app_colored.log.1) in '{temp_log_dir}'.")
    ...
    test_config_3 = {
        "log_file": os.path.join(temp_log_dir, "cli_override_colored.log"),
        "log_level": "INFO", # Config says INFO
        "log_color": True # Explicitly enable color
    }
    # CLI overrides to WARNING
    ...
    print(f"Check {os.path.join(temp_log_dir, 'cli_override_colored.log')}.")


    print("\n--- Test 4: Invalid Log Level in Config - Console Colored ---")
    test_config_4 = {
        "log_level": "INVALID_LEVEL",
        "log_file": os.path.join(temp_log_dir, "invalid_level_colored.log")
    }
    setup_logging(test_config_4) # Should default to INFO and print a warning to stderr
    logging.info("This is an INFO message (Test 4 - should see, after warning about invalid level).")
    print(f"Check {os.path.join(temp_log_dir, 'invalid_level_colored.log')}.")

    print("\n--- Test 5: Color explicitly disabled in config ---")
    test_config_5 = {
        "log_level": "INFO",
        "log_color": False, # Disable color via config
        "log_file": os.path.join(temp_log_dir, "no_color_config.log"),
    }
    setup_logging(test_config_5)
    logging.info("This is an INFO message (Test 5 - should see, NO COLOR from config).")
    print(f"Check {os.path.join(temp_log_dir, 'no_color_config.log')}.")

    print("\n--- Test 6: Color disabled by global flag (e.g. if not a TTY conceptually) ---")
    test_config_6 = {
        "log_level": "INFO",
        # "log_color": True, # Config might say true
        "log_file": os.path.join(temp_log_dir, "no_color_global_flag.log"),
    }
    setup_logging(test_config_6, use_color=False) # Global flag disables color
    logging.info("This is an INFO message (Test 6 - should see, NO COLOR from global flag).")
    print(f"Check {os.path.join(temp_log_dir, 'no_color_global_flag.log')}.")
    ...
    print("\nLogging setup tests finished. Manual cleanup of 'temp_logs_colored' directory might be needed.")
# The following section was erroneously duplicated at the end of the __main__ block in the prompt's file version.
# It is part of the setup_logging function and should not be here. Removing it.
#
#    if log_file_path:  <-- This entire block was the duplicated part.
#        try:
#            # Ensure log directory exists
#            log_dir = os.path.dirname(log_file_path)
#            if log_dir and not os.path.exists(log_dir):
